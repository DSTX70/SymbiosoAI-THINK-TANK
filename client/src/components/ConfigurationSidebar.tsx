  ...
  return (
    <div className="space-y-4">
      ...
      <SectionCard id="reasoning" title="Reasoning Frameworks" icon={Brain}>
        <div className="space-y-3">
          {['systems', 'design', 'first_principles', 'dialectical'].map((framework) => (
            <div key={framework} className="flex items-center space-x-2">
              <Checkbox
                id={framework}
                checked={configuration.frameworks.includes(framework)}
                onCheckedChange={(checked) => {
                  const newFrameworks = checked
                    ? [...configuration.frameworks, framework]
                    : configuration.frameworks.filter(f => f !== framework);
                  updateConfig('frameworks', newFrameworks);
                }}
                data-testid={`checkbox-framework-${framework}`}
              />
              <Label htmlFor={framework} className="text-sm capitalize">
                {framework.replace('_', ' ')}
              </Label>
            </div>
          ))}
      ...
      {/* Agent Routing */}
      <SectionCard id="routing" title="Agent Routing Weights" icon={Target}>
        <div className="space-y-4">
