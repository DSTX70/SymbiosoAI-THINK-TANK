...
export default function PricingUpsell(){
  ...
  return <div data-testid="pricing-upsell"><h2>Plans & Trials</h2><pre>{JSON.stringify(plans,null,2)}</pre><div>Trial: {trial?.active? `${trial.daysRemaining} days left` : <button onClick={start}>Start Trial</button>}</div></div>;
}