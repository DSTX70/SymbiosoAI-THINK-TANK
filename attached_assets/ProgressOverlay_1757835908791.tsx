...
export default function ProgressOverlay({ progress, status }: Props) {
  ...
  return (
    <div data-testid="progress-overlay" className="fixed inset-0 bg-black/40 flex items-center justify-center">
      <div className="bg-white rounded-xl p-6 w-[320px] shadow-lg">
        <div className="mb-2 font-semibold">Processing…</div>
        <div className="w-full h-2 bg-gray-200 rounded">
          <div className="h-2 rounded" style={{ width: `${Math.max(0, Math.min(100, progress))}%`, background: '#444' }} />
        </div>
